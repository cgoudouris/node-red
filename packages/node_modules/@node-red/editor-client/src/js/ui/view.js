...
RED.view = (function () {
    ...
    function showQuickAddDialog(options) {
        ...
        var rebuildQuickAddLink = function () {
            console.log('rebuildQuickAddLink');
            if (!quickAddLink) {
                return;
            }
            if (!quickAddLink.el) {
    ...
    function portMouseUp(d, portType, portIndex, evt) {
        console.warn("portMouseUp", mouse_mode, d, portType, portIndex);
            ...
            for (i = 0; i < drag_lines.length; i++) {
                if (portType != drag_lines[i].portType && mouseup_node !== drag_lines[i].node) {
                    ...
                    if (drag_line.virtualLink) {
                        ...
                        // PORT_TYPE_OUTPUT=0
                        // PORT_TYPE_INPUT=1
                        console.log('DragLine', drag_line);
                        console.log('Mouse up', mouseup_node);
                        if (!(
    ...
    function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function (d, i) {
                ...
                var isSubflow = (d.type.startsWith("subflow"));
                ...
                if (isSubflow) {
                    console.log('Subflow:', d.name, 'Object:', d);
                    console.log('Wspc:', d.type.substring(8));
                }
                ...
                if (isSubflow) {
                    ...
                    for (let side = 0; side < 7; side++) {
                        const angle_deg = 60 * side;
                        const angle_rad = Math.PI / 180 * angle_deg;
                        //points.push(`${4 * node_height + 4 * node_height * Math.cos(angle_rad)},${4 * node_height + 4 * node_height * Math.sin(angle_rad)}`);
                        points.push(`${2 * node_height + 2 * node_height * Math.cos(angle_rad)},${2 * node_height + 2 * node_height * Math.sin(angle_rad)}`);
                    }

                    let subflowzId = d.type.substring(8);
                    let nodesInSubflow = [];
                    console.log('Z id:', subflowzId);

                    RED.nodes.eachNode(function(node) {
                        console.log('Node list', node);
                        if (node.z === subflowzId) {
                            nodesInSubflow.push(node);
                        }
                    });
                    // Now, nodesInSubflow contains all nodes within the specified subflow
                    console.log('Subflow Nodes in z:',  nodesInSubflow);

                
                    //console.log('Subflow Nodes:', RED.nodes.allNodes.allNodes.getNodes());

                    // Create SVG and hexagon elements
                    //var mainRect = document.createElementNS("http://www.w3.org/2000/svg", "svg");
                    //mainRect.setAttribute('viewBox', `0 0 ${node_height} ${node_height}`);
                    mainRect = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
                    mainRect.setAttribute('points', points.join(' '));
                    // Set hexagon style
                    //mainRect.style.fill = '#aaa';
                    //mainRect.style.stroke = '#000';

                    // Add elements to DOM
                    //mainRect.appendChild(hexagon);
                    //mainRect.style.position = "absolute";
                    mainRect.style.width = `${10 * node_height}px`;
                    mainRect.style.height = `${10 * node_height}px`;
                    //svg.style.top = `${y}px`;
                    //svg.style.left = `${x}px`;
                    // svg.appendChild(div);
                    //document.getElementById('hexagons').appendChild(svg);
                    ...
                    if (d._def.icon) {
                        ...
                        icon_groupEl.setAttribute("visibility", 'hidden'); // Always hidden 
                    ...
                    text.setAttribute("transform", "translate(50,0)");
            ...
            node.each(function (d, i) {
                ...
                if (d.dirty) {
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (isSubflow) {
                            //console.log('Subflow inputs:', d.inputs);

                            var numInputs = d.inputs;

                            console.log('d:', d, d.h, this.__inputs__.length);
                            ...
                            for (var portIndex = 0; portIndex < numInputs; portIndex++) {
                                ...
                                //console.log('==> Loop:', portIndex, numInputs);

                                var inportGroup;
                                if (true) {
                                    inportGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
                                    inportGroup.setAttribute("class", "red-ui-flow-port-input");
                                    var iportPort;
                                    iportPort = document.createElementNS("http://www.w3.org/2000/svg", "rect");
                                    iportPort.setAttribute("rx", 3);
                                    iportPort.setAttribute("ry", 3);
                                    iportPort.setAttribute("width", 10);
                                    iportPort.setAttribute("height", 10);
                                    iportPort.setAttribute("class", "red-ui-flow-port");
                                    inportGroup.appendChild(iportPort);
                                    inportGroup.__port__ = iportPort;
                                    iportPort.__data__ = this.__data__;
                                    iportPort.__portType__ = PORT_TYPE_INPUT;
                                    iportPort.__portIndex__ = portIndex;
                                    iportPort.addEventListener("mousedown", portMouseDownProxy);
                                    iportPort.addEventListener("touchstart", portTouchStartProxy);
                                    iportPort.addEventListener("mouseup", portMouseUpProxy);
                                    iportPort.addEventListener("touchend", portTouchEndProxy);
                                    iportPort.addEventListener("mouseover", portMouseOverProxy);
                                    iportPort.addEventListener("mouseout", portMouseOutProxy);

                                    this.appendChild(inportGroup);
                                    this.__inputs__.push(inportGroup);
                                    RED.hooks.trigger("viewAddPort", { node: d, el: this, port: inportGroup, portType: "input", portIndex: portIndex });    
                                } else {
                                    inportGroup = this.__inputs__[portIndex];
                                }

                                var x = d.w - 120;
                                var y = (d.h / 2) - ((numInputs - 1) / 2) * 13;
                                inportGroup.setAttribute("transform", "translate(" + x + "," + ((y + 20 * portIndex) - 5) + ")");
                            }
                        } else {
            ...
            groupEnter.each(function (d, i) {
                ...
                for (let side = 0; side < 7; side++) {
                    ...
                    const angle_rad = Math.PI / 180 * angle_deg;
                    //points.push(`${4 * node_height + 4 * node_height * Math.cos(angle_rad)},${4 * node_height + 4 * node_height * Math.sin(angle_rad)}`);
                    points.push(`${2 * 50 + 2 * 50 * Math.cos(angle_rad)},${2 * 50 + 2 * 50 * Math.sin(angle_rad)}`);
                }
                ...
                var selectGroup = groupSelectLayer.append('g').attr("id", "group_select_" + d.id);

                selectGroup.append('polygon')
                   .classed("red-ui-flow-group", true)
                   .attr('points', points.join(' '));

                const groupBackground = selectGroup.append('polygon')
                    .classed("red-ui-flow-group-outline-select", true)
                    .classed("red-ui-flow-group-outline-select-background", true)
                    .attr('points', points.join(' '));
                ...
                g.append('polygon')
                    .classed("red-ui-flow-group-outline", true)
                    .attr('points', points.join(' '));
                ...
                g.append('polygon')
                    .classed("red-ui-flow-group-body", true)
                     .attr('points', points.join(' '))
                    .style({
                        "fill": d.fill || "none",
                        "stroke": d.stroke || "none",
